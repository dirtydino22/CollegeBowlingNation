(function() {
    ...
    module.exports = function(app) {
        ...
        app.get('/app.appcache', function(req, res) {
            ...
            res.sendfile('app.appcache', {'root': __dirname + '/../../app'});
        });
         ...
         */
        // api namespace
        app.namespace('/api/:token', function() {
            // api/status
            app.get('/status', function(req, res) {
                res.json(200, 'Status OK');
            });
            app.post('/newseason', UserCtrl.newSeason);
            ...
            app.namespace('/bowlers', function() {
                // api/bowlers
                app.get('/:id', BowlerCtrl.bowlers);
                app.post('/:id/', BowlerCtrl.createBowler);

                
                // api/bowlers/:bowlerId
                //app.get('/:bowlerId', BowlerCtrl.bowler);
                //app.post('/:bowlerId/:id', BowlerCtrl.newGame);
                //app.post('/:bowlerId', BowlerCtrl.updateBowler);
                //app.delete('/:bowlerId', BowlerCtrl.deleteBowler);
            });
            app.post('/newgame/:bowlerId/:id', BowlerCtrl.newGame);
            // api/teams namespace
